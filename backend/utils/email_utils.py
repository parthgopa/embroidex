...
def send_password_reset_otp_email(to_email, otp_code, user_name="User"):
    """
    Send a password reset verification code using Gmail SMTP.
    """
    try:
        sender_email = os.getenv("SMTP_EMAIL", "embroidex0910@gmail.com")
        password = os.getenv("SMTP_PASSWORD", "fgfjouuugjomtfqi").replace(" ", "")
        ...
        msg = MIMEMultipart("alternative")
        msg["Subject"] = f"{otp_code} is your Embroidex password reset code"
        msg["From"] = f"Embroidex <{sender_email}>"
        msg["To"] = to_email
...
We received a request to reset your password for your Embroidex account.
    ...
    <tr>
      <td align="center">
        <table width="100%" max-width="500" style="max-width: 500px; background-color: #ffffff; border-radius: 12px; border: 1px solid #e2e8f0; box-shadow: 0 4px 16px rgba(0, 0, 0, 0.05); overflow: hidden; padding: 0;" border="0" cellspacing="0" cellpadding="0">
          <!-- Header -->
          <tr>
            <td style="background: linear-gradient(135deg, #2563eb 0%, #4f46e5 100%); padding: 28px 30px; text-align: center;">
              <h1 style="color: #ffffff; margin: 0; font-size: 24px; font-weight: 700; letter-spacing: -0.5px;">Embroidex</h1>
              <p style="color: #e0e7ff; margin: 6px 0 0 0; font-size: 14px;">Password Reset Request</p>
            </td>
          </tr>
          <!-- Body -->
          <tr>
            <td style="padding: 35px 30px;">
              <h2 style="color: #0f172a; margin: 0 0 12px 0; font-size: 18px; font-weight: 600;">Hello, {user_name}</h2>
              <p style="color: #475569; font-size: 14px; line-height: 1.5; margin: 0 0 24px 0;">
                We received a request to reset the password for your Embroidex account. Use the verification code below to proceed with resetting your password:
              </p>
              
              <!-- OTP Box -->
              <div style="background-color: #eff6ff; border: 2px dashed #93c5fd; border-radius: 10px; padding: 18px; text-align: center; margin-bottom: 24px;">
                <span style="font-family: 'Courier New', Courier, monospace; font-size: 32px; font-weight: 800; letter-spacing: 8px; color: #2563eb; display: inline-block;">
                  {otp_code}
                </span>
                <p style="color: #64748b; font-size: 12px; margin: 8px 0 0 0;">Valid for 10 minutes</p>
              </div>
              ...
              <div style="background-color: #fef2f2; border-left: 4px solid #ef4444; padding: 12px 16px; border-radius: 4px; margin-bottom: 20px;">
                <p style="color: #991b1b; font-size: 13px; margin: 0; line-height: 1.4;">
                  <strong>Important:</strong> Never share this OTP with anyone. Embroidex staff will never ask for your verification code.
                </p>
              </div>
          ...
          <tr>
            <td style="background-color: #f8fafc; padding: 20px 30px; text-align: center; border-top: 1px solid #f1f5f9;">
              <p style="color: #94a3b8; font-size: 12px; margin: 0;">
                © 2026 Embroidex Security. All rights reserved.
              </p>
            </td>
          </tr>
        </table>
