/**
 * Lightweight, zero-dependency pure JavaScript HMAC-SHA256 implementation (RFC 2104).
 * Used for development / test payment verification when native binary SDK is in offline mode.
 */
...
export function hmacSha256(keyStr, msgStr) {
  let key = strToUtf8Bytes(keyStr);
  const msg = strToUtf8Bytes(msgStr);
  if (key.length > 64) key = sha256Bytes(key);
  while (key.length < 64) key.push(0);
  const oKeyPad = key.map((b) => b ^ 0x5c);
  const iKeyPad = key.map((b) => b ^ 0x36);
  const inner = sha256Bytes([...iKeyPad, ...msg]);
